...
CREATE TABLE IF NOT EXISTS referral_feedback (
    ...
    referral_id  UUID        NOT NULL UNIQUE REFERENCES referral_requests(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_feedback_referrer ON referral_feedback (referrer_id);

-- Add feedback-based reputation counters to users
ALTER TABLE users
